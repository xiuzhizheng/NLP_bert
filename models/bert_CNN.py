...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.dropout = 0.1
        self.cut_ratio = 0.95  # 90%作为训练集，10%作为验证集
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.OCNLI_fc = nn.Linear(config.num_filters * len(config.filter_sizes), 3)
        self.OCEMOTION_fc = nn.Linear(config.num_filters * len(config.filter_sizes), 7)
        self.TNEWS_fc = nn.Linear(config.num_filters * len(config.filter_sizes), 15)
